scoreUplift <- function(data, treat, outcome, model, nb_group=10, plotit=FALSE){
  ...
  if (plotit == TRUE) {
    qiniCurve(perf, title = "Qini Curve for proposed method model")
    qiniBarPlot(perf, title = "Uplift Bar Plot for proposed method model")
  }
  qini <- qiniArea(perf)
